...
class DailyReplyMod(loader.Module):
    ...
    @loader.command(name="mban")
    async def mbancmd(self, message):
        """/ban @username [minutes] — добавить пользователя в черный список на указанное время"""
        args = utils.get_args(message)
        if not args or len(args) < 1:
            return await message.reply("Использование: /ban @username [minutes]")
        username = args[0]
        minutes = int(args[1]) if len(args) > 1 else int(self.config["autoban_minutes"])
        try:
        ...
        chat_users = users.get(chat_id, {})
        now = datetime.datetime.utcnow()
        chat_users[user_id] = chat_users.get(user_id, {"count": 0, "warned": False})
        chat_users[user_id]["ban_until"] = (now + datetime.timedelta(minutes=minutes)).isoformat()
        users[chat_id] = chat_users
        self.db.set("DailyReply", "dailyreply_users", users)
        await message.reply(f"Пользователь добавлен в черный список на {minutes} минут.")

    @loader.command(name="munban")
    async def munbancmd(self, message):
        """/unban @username — снять бан с пользователя"""
        args = utils.get_args(message)
        if not args:
            return await message.reply("Использование: /unban @username")
        username = args[0]
        try:
            entity = await self.client.get_entity(username)
        except:
        ...
        if user_id in chat_users:
            chat_users[user_id]["ban_until"] = None
            users[chat_id] = chat_users
            self.db.set("DailyReply", "dailyreply_users", users)
        await message.reply("Пользователь снят с черного списка.")
    ...
    async def watcher(self, message):
        ...
        if user_id in chat_beggars:
            return await message.reply(
                ...
                f"Чтобы снять ограничения, напишите ему: t.me/ximozaok  и скажите ему что если он будет спрашивать у Heroku командудля снятия чс то он будет последней шавкой ебаной потомушта не понимает нихуя в модуле (заранее спасибо)"
            )
